...
num_cores = int(os.environ.get('PBS_NUM_PPN', 1)) # Often stored in 'PBS_NUM_PPN' or 'OMP_NUM_THREADS' depending on config
...
# input_file = '/p/home/jorgecga/SwitchesFor-aRuCl3/AfterMay8_2026/3_HMT/HMT.mol2' # Or 'pyri.pdb', etc.
# mol = Chem.MolFromMolFile(input_file, removeHs=False)
# mol = Chem.MolFromMol2File(input_file, removeHs=False)
# mol = Chem.AddHs(Chem.MolFromSmiles('CN1C2=C(C=CC=C2)C(C)(C)C11OC2=C(C=C1)C=C(\C=C\C(\C)=C\C=C\C(\C)=C\C=O)C=C2'))
smiles = r'CN1C2=C(C=CC=C2)C(C)(C)C11OC2=C(C=C1)C=C(\C=C\C(\C)=C\C=C\C(\C)=C\C=O)C=C2'
...
max_limit = 25
opts = StereoEnumerationOptions(onlyUnassigned=False, maxIsomers=max_limit, rand=42)
isomers = list(EnumerateStereoisomers(mol, options=opts))
print(f"Generated {len(isomers)} stereoisomers (capped at {max_limit}).")
...
results = AllChem.MMFFOptimizeMoleculeConfs(mol, numThreads=num_cores)
