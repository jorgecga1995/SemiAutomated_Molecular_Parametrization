...
geom_directory = "/p/home/jorgecga/SwitchesFor-aRuCl3/AfterMay8_2026/0_9/5_HAP/1_DFT_opt/conformer1/"
geom_file = "CoordsAfter25iter.xyz" 
...
# --- Comment if original molecule in 1e-10 m ---------------------------------- #
Bohr_geom_asumedA = np.array(mol.geometry())
TrueBohr_geom = 0.529177*Bohr_geom_asumedA
#                .529177210544
mol.set_geometry(psi4.core.Matrix.from_array(TrueBohr_geom))
# ------------------------------------------------------------------------------ #    
...
psi4.set_output_file('0.9_OptFreq_out.dat', False) 
...
psi4.set_options({
    ...
    'dft_spherical_points': 434, # 6 14 26 38 50 74 86 110 146 170 194 230 266 302 350 434 590 770 974 1202 1454 1730 2030 2354 ... https://psicode.org/psi4manual/master/dft.html#grid-selection
    'dft_radial_points': 85,     # integers in the 50 - 100 range generally
    'dft_pruning_scheme': 'robust',     # Generally safe and will speed things up

    'g_convergence': 'QCHEM',
    'flexible_g_convergence': True,
    'max_energy_g_convergence': 0.000002,
    'max_force_g_convergence': 0.00045,
    'max_disp_g_convergence': 0.0015,
    'geom_maxiter': 800,
    'full_hess_every': 1,                # -- Hessian Computation -- 1 for every step, 0 for initial only, or N for every N steps
...
while not GeomConverged and Opt_Freq_iterations < 10:
    # ====================================================================================================================================================
    optimized_energy_xyz = psi4.optimize('b973c', molecule=mol)
    print(f"Opt Done. Electronic Energy after optimization {Opt_Freq_iterations}: {optimized_energy_xyz:.8f} Hartrees")
    DFT_OptGeom_file = f'0.9_mol_afterB97-3c_Opt{Opt_Freq_iterations}.xyz'
    mol.save_xyz_file(DFT_OptGeom_file, 1)

    # ====================================================================================================================================================
    print("\nStarting Frequency Calculation")
    freq_energy, wfn = psi4.frequency('b973c', molecule=mol, return_wfn=True)   # Run the frequency calculation
    ...
    if not imaginary_freqs:
        ...
        IM_file = f"0.9_imaginary_mode_displacement_atB97-3c_{Opt_Freq_iterations}.txt"  # Define the filename
