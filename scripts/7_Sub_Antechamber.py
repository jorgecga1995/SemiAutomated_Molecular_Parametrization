...
MOLECULE_LIST_FILE = "MP2_ESP_calcToDo_Jun14.txt"
...
def submit_jobs():
    ...
    for mol_id, _, conformers in molecules:
        ...
        for conf in conformers:
            ...
            sub_script_path = os.path.join(target_dir, "1_AntechamberSub.sh")


            log_file = os.path.join(target_dir, f"{job_name}.log")
            ...
            if not os.path.exists(log_file):
                print(f"❌ Conformer {conf} of Molecule {mol_id}: MP2 ESP log file not found at {target_dir}")
                missing_count += 1
                continue
