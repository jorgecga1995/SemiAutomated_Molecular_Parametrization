...
MOLECULE_LIST_FILE = "Opt_NegChargBuildInput.txt"
...
%nprocshared=92
%mem=350Gb
#P M062X/6-311+G(2df,p) opt freq 

Optimization of {job_name}    
...
#Scenario A: The directory already fully exists  -> bc exist_ok=True is set, it does absolutely nothing and moves to the next line of code without raising an error.
#Scenario B: The directory does not exist at all	Python will create all the nested folders in the path from scratch 
#Scenario C: Only a part of the path exists (e.g., ./20_29/24_S01/1_DFT_opt/ already exists)	Python detects which parts of the path are already present and only creates the missing subdirectories (in this case, it will leave 1_DFT_opt untouched and only create conformer0 and its subfolder Gauss).
